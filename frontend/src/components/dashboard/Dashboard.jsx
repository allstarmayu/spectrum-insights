...
export default function Dashboard() {
  ...
  // Fetch topic keywords when active topic changes
  useEffect(() => {
  ...
  useEffect(() => {
    const fetchTrends = async () => {
      if (!activeTopicId || topicKeywords.length === 0) return;
    ...
    fetchTrends();
  }, [activeTopicId, timeframe, topicKeywords]);
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-6">
              <h2 className="text-2xl font-bold text-gray-900 mb-2">
                {activeTopicId?.split('_').map(word => 
            ...
            {/* Raw Data Preview (temporary - we'll replace with charts) */}
            <div className="bg-white rounded-lg border border-gray-200 p-6">
        ...
        {!loading && !error && !trendsData && (
          <div className="bg-white rounded-lg border border-gray-200 p-12 text-center">
            <p className="text-gray-600">Select a topic to view trends data</p>
          </div>
        )}
      </main>
    </div>
